...
struct Softmax {
  ...
  __forceinline__ __device__ TensorT max_get_scale(Tensor0& acc_s) {
    ...
    Tensor scores = make_tensor(
        acc_s.data(), hstu::convert_layout_acc_rowcol(acc_s.layout()));
    static_assert(CUTE_STATIC_V(size<0>(scores)) == kNRows);
    TensorT scores_scale;
    if constexpr (Is_first) {
      hstu::template reduce_max</*zero_init=*/true>(scores, row_max);
      cute::fill(scores_scale, 1.f);
    } else {
      Tensor scores_max_prev = make_fragment_like(row_max);
      cute::copy(row_max, scores_max_prev);
      hstu::template reduce_max</*zero_init=*/false>(scores, row_max);
#pragma unroll
  ...
  __forceinline__ __device__ void online_softmax(Tensor0& acc_s) {
    ...
    Tensor scores = make_tensor(
        acc_s.data(), hstu::convert_layout_acc_rowcol(acc_s.layout()));
    static_assert(CUTE_STATIC_V(size<0>(scores)) == kNRows);
    hstu::template scale_apply_exp2</*Scale_max=*/true, Check_inf, Max_offset>(
    ...
    // softmax.
    hstu::reduce_sum</*zero_init=*/Is_first, /*warp_reduce=*/false>(
        scores, row_sum);
  };
...
#pragma unroll
  ...
  __forceinline__ __device__ void rescale_o(
    ...
    Tensor acc_o_rowcol = make_tensor(
        acc_o.data(), hstu::convert_layout_acc_rowcol(acc_o.layout()));
