...
def train(word_dict,
    ...
    # lod_level stores the offset for the mini_batch. An abstraction on top of tensors to store variable
    # length sequences.
    ...
    #adam = fluid.optimizer.Adam(learning_rate=0.0009)

    """
    Learning rate of 5e-4 works for conv models and 2e-3 for LSTM models
    """
    adam = fluid.optimizer.Adam(learning_rate=0.0005)

    #adam = fluid.optimizer.Adam(learning_rate=0.002, regularization=fluid.regularizer.L2DecayRegularizer(regularization_coeff=0.00005))
    ...
    def train_loop(main_program):
        ...
        for pass_id in xrange(PASS_NUM):
            train_loss_set = []
            train_acc_set = []     
            # Calcuate training loss and accuracy
	    ...
	    train_loss = np.array(train_loss_set).mean()
            ...
            # Calculate valication loss and accuracy
            acc_set = []
            avg_loss_set = []
