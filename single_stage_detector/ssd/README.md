...
2. _Hard negative mining_ in the loss function.  (implemented with multiple kernels in `opt_loss.py`).  The ground-truth tells you which anchors are assigned as "postives", but most anchors will be negatives and so would overwhelm the loss calculation, so we need to choose a subset to train against.
