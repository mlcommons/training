...
The ResNet34 backbone is initialized with weights from PyTorch hub:
...
The conv4 layers consist of 6 building blocks.  The very first convolution in the conv4 layers doubles the number of channels to 256, so these convolutions all output 38x38x256 activations.  The identity connection on this initial building block is a 1x1 unstrided convolution.  (This is an artifact of the way the backbone was derived from the original resnet34 architecture).

The backbone is initialized with the pretrained weights from the [Torchvision model zoo](https://download.pytorch.org/models/resnet34-333f7ec4.pth), described in detail [here](https://pytorch.org/docs/stable/torchvision/models.html) (This is a resnet34 network trained on Imagenet to achieve a Top-1 error rate of 26.7 and a Top-5 error rate of 8.58.)
...
In the deployed _inference_ network non-maximum suppression is used to detect which anchor points "contain" an object of class x.  Correctness is evaluated by comparing the Jaccard overlap of the predicted anchor box offsets with the ground truth.  The _training script_ must associate ground truth with anchor boxes:

1. Calculate _ground truth_ from the list of bounding boxes (this is implemented in function box_encoder in `csrc/box_encoder_cuda.cu`):

    a. Calculate the Jaccard overlap of every anchor's default position with each ground truth bounding box
...
2. _Hard negative mining_ in the loss function.  (implemented with multiple kernels in `opt_loss.py`).  The ground-truth tells you which anchors are assigned as "positives", but most anchors will be negatives and so would overwhelm the loss calculation, so we need to choose a subset to train against.
    ...
    code. Thus most anchors will not be associated with any ground truth, so will have dgrad of 0, and won't get trained in this step.

3. For each assigned anchor: The loss over categories is the softmax loss over class confidences.  The loss over offsets is the Smooth L1 loss between the predicted box and the ground truth box.  (These losses are implemented just after hard negative mining in `opt_loss.py`)
...
3. color jitter using `torchvision.transforms.ColorJitter`(brightness=0.125, contrast=0.5, saturation=0.5, hue=0.05). (brightness adjusted by factor chosen uniformly between (.875, 1.125), contrast adjusted by factor chosen uniformly between 0.5, 1.5, saturation adjusted by factor chose uniformly from 0.5 to 1.5, hue adjust by factor chosen uniformly from -0.05 to 0.05 (where hue is an angle such that -0.5 is -180 degrees and 0.5 is 180 degrees, so this is +or-18 degrees.)

## Publication/Attribution
_Wei Liu, Dragomir Anguelov, Dumitru Erhan, Christian Szegedy, Scott Reed, Cheng-Yang Fu, Alexander C. Berg. SSD: Single Shot MultiBox Detector. In the Proceedings of the European Conference on Computer Vision (ECCV), 2016_
