...
def parse_args():
    parser = ArgumentParser(description="Read a MovieLens ratings.csv and add fake users"
                                        "This will be done by duplicating entries and changing user id")
    parser.add_argument('filename', type=str,
                        help='filename of input file (should be ratings.csv')
    parser.add_argument('multiplier', type=int,
                        help='how many times to duplicate each user. Each duplicated user id will be original user id+x000000 where x=duplication number')
...
def main():
    ...
    output_filename=os.path.splitext(input_filename)[0]+"_users_expanded.csv"
    ...
    for row_index,row in enumerate(csv_reader):
        ...
        if not header:
            for i in range(1,multiplier):
                new_row=row.copy()
                original_user_id=int(row[0])
                new_user_id=original_user_id+1000000*i
                new_row[0]=str(new_user_id)
